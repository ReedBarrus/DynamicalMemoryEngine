...
| `TRJ-APP-001` | `App surface remodeling` | Move toward a thin router/home shell above dedicated live, static, and inspection viewers while keeping structural evidence primary and overlays subordinate. | `Leg 1 primary; depends into Legs 2 and 3` | `active` | `PKT-REORG-CLOSEOUT-AUDIT-007`; `PKT-ACTIVE-TRAJECTORIES-008` | `PKT-ACTIVE-TRAJECTORIES-008` (accounting only) | `dedicated app-surface routing/viewer packet` | `README/Diagnostics/AppSurface/README.AppSurfaceTransitionPlan.v0.md`; current HUD/app surfaces; semantic overlay contract posture | router/viewer split still needs bounded implementation packets; source-selection/UI coupling must remain below truth posture | no roadmap rewrite; no styling-first rewrite; no overlay authority uplift | park when the thin router and dedicated viewer split are stable enough that later packets are mostly incremental surface hardening |
