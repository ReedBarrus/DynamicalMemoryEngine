...
| `Transformer/LanguageKernel/invocation/` | `Transformer/LanguageKernel/invocation/` | invocation_subgroup | active | cross-door | seam-local Ollama prompt build, response capture, receipt-chain mechanization, run assembly, and bounded two-model comparison summary emission | `README/Transformer/LanguageKernel/README.LanguageKernelContract.v0.md`; `README/Transformer/LanguageKernel/README.LanguageKernelEarlyOperation.v0.md`; `README.RepoPlacementConstitution.md` | medium | Houses the bounded local-model invocation runner and its same-input compare-panel helper only. It does not become repo-global model orchestration or runtime infrastructure. |
| `Transformer/LanguageKernel/outputs/` | `Transformer/LanguageKernel/outputs/` | generated_example_bank | active | cross-door | bounded Language Kernel receipts, run objects, benchmark outputs, manual/real invocation captures, and comparative panel artifacts | `README/Transformer/LanguageKernel/README.LanguageKernelBenchmarkContract.v0.md`; `README.RepoPlacementConstitution.md` | medium | Mixed seam-local generated surface used for examples and verification. It now includes a refreshed Hermes real first-run chain, blocked Meta invocation artifacts when local model availability fails, and a comparative panel summary; all remain audit/evaluation outputs rather than authority objects. |
...
| `tests/hud/app_surfaces/` | `tests/hud/app_surfaces/` | verification_subgroup | active | door_one | HUD app-surface routing, mode-shell, state-threading, live-telemetry, live/static spectral-view, static energy-view, and intake verification | `README.RepoPlacementConstitution.md`; `README.WorkflowContract.md` | strong | High-confidence app-surface tests now grouped by verification function rather than future-door rhetoric; current coverage includes router-shell, explicit mode-shell split, published shell-state threading, Live-only telemetry rail verification, the first Live continuous structural face, the first Static spectral face, and the new Static energy face. |
...
| `hud/ViewerModeShellFrame.jsx`; `hud/LiveModeShell.jsx`; `hud/StaticModeShell.jsx`; `hud/InspectionModeShell.jsx`; `hud/LiveContinuousStructuralViewer.jsx`; `hud/StaticSpectralViewer.jsx`; `hud/StaticEnergyViewer.jsx` | `hud/` | viewer_mode_shell_family | active | door_one | explicit Live / Static / Inspection route shells over one shared payload seam | `README/Diagnostics/AppSurface/README.LiveStaticInspectionSplit.v0.md`; `README/Diagnostics/AppSurface/README.StructuralViewerContract.v0.md`; `README/Diagnostics/AppSurface/README.LiveViewerTelemetryContract.v0.md`; `README/Diagnostics/AppSurface/README.AppSurfaceTransitionPlan.v0.md` | medium | Mode-shell family remains in the existing hud zone without creating a new folder; Live mode contains one real continuous structural viewer and Static mode now contains bounded spectral and energy counterparts while Inspection remains shell-level. |
