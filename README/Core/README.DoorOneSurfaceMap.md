...
For top-level project orientation, see:
- `README.md`

For the current Door One freeze-line and explicit “not required yet” posture, see:

- `README_DoorOneAcceptanceChecklist.md`

This Surface Map owns compact structural accounting.
The Acceptance Checklist owns current-phase sufficiency.

For read-side evidence-order and anti-overread law governing these inspection surfaces, see:
...
A. Lower runtime spine
raw ingest / lawful flush input
DoorOneExecutiveLane
DoorOneOrchestrator
CrossRunSession
DoorOneWorkbench

This remains the lower operational chain and should stay. It’s still consistent with the acceptance checklist and runtime-seam posture.

B. Current app/operator surface
SemanticOscilloscopeApp
MetaLayerObjectExecutionShell
app_main.jsx
execution_main.jsx
demo_main.jsx
replay preparation seam (replayModel.js)
request preparation seam (requestModel.js)
active replay rendering seam (ReplayRegion.jsx)
lab HUD
tandem/public shaping
ingest adapters / tandem adapter
...
## 4.5 App/Composition surfaces

A. SemanticOscilloscopeApp / entry surfaces

Role:

- app-entry composition and mode posture
- not runtime authority
- not full backend maturity proof

B. MetaLayerObjectExecutionShell

Role:

- operator harness for source selection, explicit run, request prep, replay prep, workbench/HUD coordination
- execution surface only, not runtime meaning

C. Request preparation surfaces
- request objects are partially mechanized: prepared, lineage-bearing, exportable, but not fulfilled consultation/review protocol.

D. Replay preparation + rendering surfaces
- replay objects and rendering are now materially stronger than before because the reconstruction backend exists, the model reaches it, and the active replay surface renders reconstruction summary, threshold posture/downgrade, trace, and explicit non-claims/failure posture.
- but the surface must stay support-trace only, lens-bound, retained-tier-honest, and below canon/truth/promotion.

E. Tandem/public shaping
- partially mechanized shaping layer, not deep evidence parity, not truth engine.

control surface is lawful if seam discipline is visible
...
- request panel,
- demo/public shaping,
- and any future browser shell surface.
...
## 6.5 Reconstruction/Replay Surfaces

Purpose:

prepare and render bounded support-trace replay under declared lens and retained-tier posture
keep reconstruction explicitly below raw restoration, truth, canon, and promotion

Current posture:

backend support-trace reconstruction exists,
replay model reaches backend,
active replay surface renders reconstruction-backed fields in lawful order,
failure and downgrade posture are explicit.

Still not claimed:

source restoration,
higher-tier replay maturity,
replay as truth,
replay as canon-support beyond declared bounded use.
...
For current declared / displayed / partially mechanized / mechanized status across these surfaces, see:

- `README.DeclaredVsMechanizedAudit.md`

This Surface Map owns compact structural accounting.
The Declared vs Mechanized Audit owns capability-status discipline.
...
This note is a compact accounting surface.
It should not be treated as a full current repo inventory by itself.
Current app/operator reality must be cross-checked against:

repo topology,
declared-vs-mechanized audit,
runtime boundary,
and the address registry entry for this note.

# Door One Probe Nexus

Refer to the Door One Probe Nexus as a compact accounting surface for probe ecology.