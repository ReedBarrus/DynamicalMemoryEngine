...
const OUTPUT_DIR = process.env.DOOR_ONE_CAPSTONE_OUTPUT_DIR
    ?? "./out_experiments/amplitude_delta12_replay_capstone";
const PROV_ROOT = process.env.DOOR_ONE_CAPSTONE_PROVENANCE
    ?? "./out_provenance/amplitude_delta12_replay_capstone";
const PIN_ROOT = process.env.DOOR_ONE_CAPSTONE_PIN_ROOT
    ?? "./out_provenance/daw_tone_amplitude_delta12_v1";
...
const BASE_POLICIES = {
    clock_policy_id: "clock.capstone.amplitude_delta12.v1",
...
async function main() {
    ...
    // ── Experiment manifest ────────────────────────────────────────────────────
    const manifest = {
        experiment_type: "door_one_amplitude_delta12_replay_capstone",
        ...
        generated_from: "door_one_amplitude_delta12_replay_capstone runner — not canon, not promotion",
    };
...
async function buildAndWritePinPacket({ provenanceRoot, pinRoot, digest, manifest }) {
    ...
    const packet = {
        ...
        packet_metadata: {
            ...
            pin_label: `daw_tone_amplitude_delta12_v1_cycles_${String(cycleMin ?? 0).padStart(4, "0")}_${String(cycleMax ?? 0).padStart(4, "0")}`,
