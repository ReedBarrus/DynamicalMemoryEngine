...
//   - build a bounded digest over receipt history
//   - write a non-canonical digest summary back to provenance storage
//
// Boundary contract:
//   - thin script-side reader only
//   - consumes durable provenance receipts as-is
//   - does not alter runtime semantics
//   - does not mint canon
//   - does not promote
//   - does not define truth or ontology
//
// References:
...
function sortTimelineRows(rows) {
    return [...rows].sort((a, b) => {
        const ai = Number(a.cycle_index ?? 0);
        const bi = Number(b.cycle_index ?? 0);
        return ai - bi;
    });
}
...
export function buildProvenanceDigest({
        ...
        .map((entry) => {
            ...
            return {
                ...
                transition_selectivity:
                    receipt?.structural_summary?.transition_selectivity ?? "unknown",
    ...
    for (let i = 1; i < timeline.length; i++) {
        ...
        if (cur.readiness !== prev.readiness) readiness_changed_count += 1;
        ...
        if (cur.consensus_result !== prev.consensus_result) consensus_changed_count += 1;
    ...
    return {
        ...
        aggregates: {
            ...
            claim_type_counts: countBy(timeline.map((r) => r.claim_type)),
...
async function main() {
    ...
    console.log(
        `  cycles: ${res.digest?.scope?.cycle_index_min ?? "—"} → ${res.digest?.scope?.cycle_index_max ?? "—"}`
    );
    console.log(
        `  readiness counts: ${JSON.stringify(res.digest?.aggregates?.readiness_counts ?? {})}`
    );
    console.log(
        `  consensus counts: ${JSON.stringify(res.digest?.aggregates?.consensus_counts ?? {})}`
    );
    console.log("");
}
