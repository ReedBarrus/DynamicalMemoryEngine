...
// CLI args
// usage example:
// node scripts/run_c1_first_consultation.js \
//   --wavDir=root/test_signal/daw_tone_sine_400hz_RoomChange \
...
const WAV_DIR = argv.wavDir
    ? path.resolve(REPO_ROOT, argv.wavDir)
    : "/mnt/user-data/uploads";

const SUFFIXES = String(argv.suffixes ?? "01,02,03")
    .split(",")
    .map((s) => s.trim())
    .filter((s) => s.length > 0);

const RUN_LABEL = argv.label ?? "c1_first_consultation";

const OUT_DIR = path.join(REPO_ROOT, "out_canon", RUN_LABEL);
...
function phaseFiles(prefix, suffixes) {
    return suffixes.map((s) => `${prefix}_${String(s).trim()}.wav`);
}
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
    ...
    const baselineFiles = phaseFiles("baseline", SUFFIXES);
    const perturbFiles = phaseFiles("perturb", SUFFIXES);
    const returnFiles = phaseFiles("return", SUFFIXES);

    console.log("Cohort directory:", WAV_DIR);
    ...
    const record = {
        record_type: "c1_consultation_result",
        record_version: "1.1",
        ...
        band_profile_metrics: {
            ...
            file_count_per_phase: baselineFiles.length,
        },
    ...
    const outPath = path.join(OUT_DIR, `${RUN_LABEL}_result.json`);
