...
const BASE_POLICIES = {
    clock_policy_id: "clock.capstone.continuity.v1",
...
async function main() {
    ...
    // ── Experiment manifest ────────────────────────────────────────────────────
    const manifest = {
        experiment_type:  "door_one_continuous_replay_capstone",
...
async function buildAndWritePinPacket({ provenanceRoot, pinRoot, digest, manifest }) {
    ...
    const packet = {
        ...
        packet_metadata: {
            ...
            pin_label:         `daw_tone_continuity_v1_cycles_${
                String(cycleMin ?? 0).padStart(4,"0")}_${String(cycleMax ?? 0).padStart(4,"0")}`,
