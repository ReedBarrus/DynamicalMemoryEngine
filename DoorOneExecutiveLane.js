 ...
 *   v0.1 supports:
 *     - synthetic input
 *     - prebuilt raw ingest input
 *
 *   Later attach points may include:
 *     - AnalogSamplerOp flush output
 *     - file / socket / device adapters
 *
 * Public API:
...
export class DoorOneExecutiveLane {
     ...
     * @param {Object} [opts.epochContext]
     * @param {Object} [opts.consensusPolicy]
    ...
    constructor(opts = {}) {
        ...
        this.epochContext = this._copy(opts.epochContext ?? null);
        this.consensusPolicy = this._copy(opts.consensusPolicy ?? null);
     ...
     * @param {Object} [opts.epochContext]
     * @param {Object} [opts.consensusPolicy]
     * @param {Object} [opts.candidateOptions]
     * @returns {Object}
     */
    ingest(rawInput, opts = {}) {
        const inputCheck = this._validateRawInput(rawInput);
        if (!inputCheck.ok) {
            return inputCheck;
        }
        ...
        const runResult = orchestrator.runBatch(rawInput, {
        ...
        const lawfulCrossRunReport =
            crossRunReport && crossRunReport.ok !== false
                ? crossRunReport
                : null;

        const workbench = this._workbench.assemble(runResult, {
            crossRunReport: lawfulCrossRunReport,
            epochContext: this._copy(opts.epochContext ?? this.epochContext),
            consensusPolicy: this._copy(opts.consensusPolicy ?? this.consensusPolicy),
            candidateOptions: this._copy(opts.candidateOptions ?? {}),
        });
