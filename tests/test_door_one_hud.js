...
const { signal } = makeTestSignal({
    durationSec: 4, fs: 8, seed: 7, noiseStd: 0.01,
...
assert("B6: panel [6] INTERPRETATION present", output.includes("[6] INTERPRETATION"));
...
assert("D: 'ontology' only in denial/boundary statements",
    !output.includes("ontology") ||
    (
        output.includes("not canon, not intent, not ontology") ||
...
// J. Interpretation
// ════════════════════════════════════════════════════════════════════════════
section("J. Interpretation panel");

assert("J1: interpretation panel shows base trajectory block",
    output.includes("base trajectory interpretation:"));

assert("J2: interpretation panel shows attention / memory block",
    output.includes("attention / memory overlay:"));

assert("J3: convergence row present",
    output.includes("convergence"));

assert("J4: attention concentration row present",
    output.includes("attention.conc"));

assert("J5: memory stability row present",
    output.includes("memory.stability"));

assert("J6: pre_commitment row present",
    output.includes("pre_commitment"));

assert("J7: interpretation note preserves boundary wording",
    output.includes("not canon, not intent, not ontology"));
