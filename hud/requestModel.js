...
function extractLineage(workbench, runResult) {
    ...
    const ingest    = runResult?.ingest?.artifact ?? {};

    return {
        run_label:            runResult?.run_label ?? null,
        stream_id:            scope?.stream_id ?? ingest?.stream_id ?? null,
        source_id:            ingest?.source_id ?? null,
        ...
        anomaly_count:        (runResult?.anomalies ?? []).length,
...
function deriveSupportBasis(workbench, runResult) {
    const basis = [];
    const ev = workbench?.runtime_evidence ?? {};
    if (ev.harmonic_state_count > 0)  basis.push("harmonic_state_evidence");
    if (ev.merged_state_count   > 0)  basis.push("merged_state_evidence");
    if (ev.query_result_count   > 0)  basis.push("query_result_evidence");
    if ((runResult?.anomalies ?? []).length > 0) basis.push("anomaly_event_evidence");
...
export function buildConsultationRequest({
    ...
    declaredLens        = "medium FFT/Hann (N=256, hop=128, Fs=2400Hz)",
    ...
    return {
        ...
        consultation_posture:    "comparison_request_only · not truth · not promotion",
        ...
        // Posture
        derived_vs_durable:      "derived · not durable until explicit review resolves this request",
...
export function buildActivationReviewRequest({
    ...
    return {
        ...
        // Posture
        derived_vs_durable:   "derived · not durable until review decision",
