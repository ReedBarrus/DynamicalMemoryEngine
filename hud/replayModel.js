...
export function declareLens(sourceFamilyLabel = "unspecified", runResult = null) {
    return {
        ...
        stream_id:          runResult?.ingest?.artifact?.stream_id ?? null,
...
function deriveReplaySupport(workbench, runResult) {
    ...
    if ((ev.harmonic_state_count ?? 0) > 0)   basis.push("harmonic_state_evidence");
    if ((ev.merged_state_count   ?? 0) > 0)   basis.push("merged_state_evidence");
    if ((ev.query_result_count   ?? 0) > 0)   basis.push("query_result_evidence");
...
export function buildRuntimeReconstructionReplay({
    ...
    return {
        ...
        // Provenance lineage
        source_family:         sourceFamilyLabel,
        stream_id:             scope?.stream_id ?? runResult?.ingest?.artifact?.stream_id ?? null,
        source_id:             runResult?.ingest?.artifact?.source_id ?? null,
...
export function buildRequestSupportReplay({
    ...
    return {
        ...
        allowed_use:           "bounded support replay for prepared request inspection only · not fulfillment · not promotion",
