...
function extractProvenance(input) {
    ...
    return {
        ...
        object_id:           safeStr(scope?.stream_id ?? ingest?.stream_id),
        source_id:           safeStr(ingest?.source_id),
        ...
        ingest_ok:           !!(runResult?.ingest?.ok),
        lineage_note:        runResult?.run_label
            ? `shell run · ${sourceFamilyLabel} · ${safeStr(scope?.stream_id)}`
            : "no run yet",
    };
}
...
function extractEvidence(input) {
    ...
    const anomalies = runResult?.anomalies ?? [];
    return {
        harmonic_state_count: safeNum(ev.harmonic_state_count),
        merged_state_count:   safeNum(ev.merged_state_count),
        query_result_count:   safeNum(ev.query_result_count),
        anomaly_count:        anomalies.length,
        anomaly_events:       anomalies.slice(0, 5).map(a => ({
    ...
    return {
        ...
        evidence: {
            ...
            evidence_summary_rows: [
                ["harmonic_states",   String(evid.harmonic_state_count)],
    ...
    const evidenceSummary = norm.hasResult
        ? [
            ...
            evid.anomaly_count > 0
                ? `${evid.anomaly_count} structural transition event${evid.anomaly_count !== 1 ? "s" : ""} observed`
                : "no structural transition events",
